...
class Subscription {
    ...
    this.getByIdAndUser(id, userId, (err, oldSubscription) => {
      ...
      const query = `
        ...
        SET name = ?, description = ?, provider = ?, amount = ?, 
            ...
            category = ?, active = ?, cancelled_at = ?, updated_at = NOW()
        WHERE id = ? AND user_id = ?
      `;
  ...
  static deleteByUser(id, userId, callback) {
    ...
    this.getByIdAndUser(id, userId, (err, subscription) => {
      ...
      const query = `UPDATE subscriptions SET active = false, updated_at = NOW() WHERE id = ? AND user_id = ?`;
  ...
  static getMonthlySpending(callback) {
    ...
    db.all(query, [], (err, rows) => {
      ...
      const details = rows.map(row => {
        let monthly_amount = parseFloat(row.amount);
        
        // 根据计费周期转换为月度金额
        switch (row.billing_cycle) {
          case 'yearly': monthly_amount = monthly_amount / 12; break;
          case 'half_yearly': monthly_amount = monthly_amount / 6; break;
          case 'quarterly': monthly_amount = monthly_amount / 3; break;
          case 'weekly': monthly_amount = monthly_amount * 4.33; break;
          case 'daily': monthly_amount = monthly_amount * 30.44; break;
        }
        
        if (!totals[row.currency]) {
          totals[row.currency] = 0;
        }
        totals[row.currency] += monthly_amount;
  ...
  static getYearlySpending(callback) {
    ...
    db.all(query, [], (err, rows) => {
      ...
      const totals = {};
      const details = rows.map(row => {
        let yearly_amount = parseFloat(row.amount);
        
        // 根据计费周期转换为年度金额
        switch (row.billing_cycle) {
          case 'monthly': yearly_amount = yearly_amount * 12; break;
          case 'yearly': yearly_amount = yearly_amount; break;
          case 'half_yearly': yearly_amount = yearly_amount * 2; break;
          case 'quarterly': yearly_amount = yearly_amount * 4; break;
          case 'weekly': yearly_amount = yearly_amount * 52; break;
          case 'daily': yearly_amount = yearly_amount * 365; break;
        }
        
        if (!totals[row.currency]) {
          totals[row.currency] = 0;
        }
        totals[row.currency] += yearly_amount;
